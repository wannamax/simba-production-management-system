...
export default function ProductionWorkflowSettings(){
  ...
  useEffect(()=>{load();},[]);
  const add=()=>{setEditing(null);form.resetFields();form.setFieldsValue({is_active:true,project_types:[],stages:[{is_required:true,tracks_quantity:true,allow_parallel:false}]});setOpen(true);};
    ...
    <Modal title={editing?'Sửa quy trình sản xuất':'Thêm quy trình sản xuất'} open={open} onCancel={()=>setOpen(false)} footer={null} width={1050} destroyOnClose>
      <Form form={form} layout="vertical" onFinish={save}>
        ...
        <Form.List name="stages">{(fields,{add,remove})=><Space direction="vertical" style={{width:'100%'}} size={12}>
          ...
          <Button block type="dashed" icon={<PlusOutlined/>} onClick={()=>add({is_required:true,tracks_quantity:true,allow_parallel:false})}>Thêm công đoạn tiếp theo</Button>
        </Space>}</Form.List>
